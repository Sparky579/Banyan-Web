...
function moveFromHeldKeys() {
  ...
  for (let player = 0; player < game.players.length; player++) {
    ...
    let direction = -1;
    if (control.joystick) { if (left && up) direction = 2; else if (right && up) direction = 3; else if (left && down) direction = 4; else if (right && down) direction = 5; else if (left) direction = 0; else if (right) direction = 1; else if (up) direction = 2; else if (down) direction = 4; }
