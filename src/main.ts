...
function drawBoard() {
  ...
  ctx.globalAlpha = 1;
  for (const c of g.cells.values()) { const p = pos(c.x, c.y), r = scale * .30, baseWidth = scale, baseHeight = scale * 292 / 252, nodeWidth = scale * 293 / 253, nodeHeight = scale; if (c.wall) continue; ctx.globalAlpha = c.owner >= 0 ? .62 : .26; ctx.drawImage(sprites.empty, p.x - baseWidth / 2, p.y - baseHeight / 2, baseWidth, baseHeight); ctx.globalAlpha = 1; if (c.owner >= 0) { const marker = c.root ? sprites.SquareRoot : sprites.node; ctx.save(); ctx.drawImage(marker, p.x - nodeWidth / 2, p.y - nodeHeight / 2, nodeWidth, nodeHeight); ctx.globalCompositeOperation = "source-atop"; ctx.fillStyle = COLORS[c.owner]; ctx.globalAlpha = c.root ? .8 : .95; ctx.fillRect(p.x - nodeWidth / 2, p.y - nodeHeight / 2, nodeWidth, nodeHeight); ctx.restore(); const blink = g.elapsed - c.reinforcedAt <= .5 && Math.floor((g.elapsed - c.reinforcedAt) * 5) % 2 === 0; ctx.fillStyle = blink ? "#2678ff" : c.nearRoot && !c.pest ? "#071326" : "#d23036"; ctx.font = `600 ${Math.max(10, scale * .18)}px Banyan`; ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.fillText(String(Math.floor(c.hp)), p.x, p.y + 1); } if (c.fruit > 0) { ctx.globalAlpha = c.fruit <= 3 && Math.floor(g.elapsed * 2) % 2 === 0 ? .18 : 1; ctx.drawImage(sprites.Fruit, p.x - r * .34, p.y - r * 1.58, r * .68, r * .68); ctx.globalAlpha = 1; } if (c.pest) { ctx.drawImage(sprites.Pest, p.x - r * .68, p.y - r * 1.45, r * 1.36, r * .72); } }
